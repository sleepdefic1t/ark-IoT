...
function findWith(hash) {
    console.log('\n **** transaction.findWith(hash): Called **** \n ..checking if transaction exists..');
    var address = String(storage.jarAddress());
    verifyTransactionsFor(address, hash);
};
function verifyTransactionsFor(address, hash) {
    request('http://35.188.148.247:4002/api/transactions?recipientId=' + address.substring(1, address.length-1),  function (error, response, body) {
